...
void TextEditor::DeleteWordLeft()
{
	...
	else
	{
		...
		if (pos.mColumn == 0)
		{
			...
			auto& currentLine = mLines[pos.mLine];
		...
		else
		{
			...
			int startIdx = cindex - 1;

			while (startIdx > 0 && isspace(line[startIdx].mChar))
				--startIdx;

			while (startIdx > 0 && !isspace(line[startIdx - 1].mChar))
				--startIdx;

			int count = cindex - startIdx;
			u.mRemovedStart = Coordinates(pos.mLine, mState.mCursorPosition.mColumn - count);
			u.mRemovedEnd = Coordinates(pos.mLine, mState.mCursorPosition.mColumn);
			u.mRemoved = GetText(u.mRemovedStart, u.mRemovedEnd);

			line.erase(line.begin() + startIdx, line.begin() + cindex);
